<?php
	...
	class postModel extends Model
	{
		...
		{
			$id = (int) $id;
			$post = $this->_db->query("select * from posts where post_id = $id");
			return $post->fetch();
		}
		...
		{
			$id = (int) $id;
			$this->_db->prepare("UPDATE posts SET titulo = :titulo, cuerpo = :cuerpo WHERE post_id = :id")
					->execute(
							array(
								':id'	  => $id,
								':titulo' => $titulo,
		...
		{
			$id = (int) $id;
			$this->_db->query("DELETE FROM posts WHERE post_id = $id");
		}
	}
?>